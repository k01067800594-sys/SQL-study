...
create table customers(
    ...
    create table orders(
        ...
        -- 상품 가격에 택배비 3000원ㅇ르 추가하여 실제구매금액 구함->상푸명, 가격, 실제구매금액
        select p_name, price, price+3000 as 실제구매금액
        from products;
        
        -- 상품가격을 10으로 나누어 10개월 무이자할부시 월납주액을 구함
        -- 상품명, 가격, 월납부액
        ...
        select concat(c_name, '(', email,')') as `이름과 메일` from customers;
        
        select email, upper(email) as 대문자메일 from customers;
        
        select c_name, char_length(c_name) as 글자수, length(c_name) as 바이트수 from customers;
        select p_name, ifnull(descr, '상품설명없음') as 설명 from products;
        -- ifnull(null이면, '상품섦여없음'반환)
        
        select email, substring_index(email, '@',1) as 아이디 from customers;